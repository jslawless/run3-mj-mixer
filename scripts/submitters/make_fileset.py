...
(/store/...) of SLIMMED files. The dataset name is derived from the filename by
stripping a trailing '_filelist' suffix and the extension. Paths are prefixed
with the XRootD redirector so condor jobs can stream files over WAN.
...
def main():
    ...
    parser.add_argument(
        "--tree", default="events",
        help="Tree name to record for every file (slimmed files use 'events')",
    )
