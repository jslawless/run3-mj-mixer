...
separates "that job was slow" from "that slice is big". Colours come from
matplotlib's `tab20`, strong hues first. Eleven colours is past what anyone
tells apart reliably, so the printed table is the authority — read counts off
it, not off the swatches. A job whose files straddle a slice boundary is
labelled `several slices` in neutral grey rather than being filed under
whichever slice came first. The slice label comes from
